...
def geoception(lr, output=1, model_name='geoception.model', device='cpu', num='0'):
    with tf.device('/{}:{}'.format(device, num)):
        network = input_data(shape=[None, 224], name='input')
...
# used in v0.03-v0.06+
def otherception3(width, height, frame_count, lr, output=260, model_name = 'otherception.model', device = 'gpu', num = '0'):
    network = input_data(shape=[width, height, 3], name='input')
    conv1_7_7 = conv_2d(network, 64, 28, strides=4, activation='relu', name = 'conv1_7_7_s2')
    pool1_3_3 = max_pool_2d(conv1_7_7, 9,strides=4)
    pool1_3_3 = local_response_normalization(pool1_3_3)
    conv2_3_3_reduce = conv_2d(pool1_3_3, 64,1, activation='relu',name = 'conv2_3_3_reduce')
